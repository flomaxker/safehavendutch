                    ...
                    <?php foreach ($nav_links as $index => $link): ?>
                        <?php if (isset($link['children']) && !empty($link['children'])): ?>
                            ...
                            <li x-data="{ open: JSON.parse(localStorage.getItem('<?php echo $menu_id; ?>')) ?? <?php echo $is_parent_active ? 'true' : 'false'; ?> }" x-init="$watch('open', val => localStorage.setItem('<?php echo $menu_id; ?>', val))">
                                <a href="#" @click.prevent="open = !open" class="flex items-center justify-between text-gray-600 hover:text-gray-900 font-medium p-2 rounded-lg transition-colors duration-200 text-sm <?php echo $is_parent_active ? 'bg-gray-100 text-gray-900' : ''; ?>">
                                ...
                                <ul x-show="open" x-transition class="pl-4 mt-2 space-y-2">
                                    <?php foreach ($link['children'] as $child): ?>
                                        ...
                                        <li>
                                            <a href="<?php echo $child['url']; ?>" class="flex items-center text-gray-600 hover:text-gray-900 font-medium p-2 rounded-lg transition-colors duration-200 text-sm pl-8 <?php echo $is_child_active ? 'bg-gray-200 text-gray-900' : ''; ?>">
